...
let form = useForm({
    grade: '',
    subject: user.value.teacher.subject.title,
    lessons: '',
    type: null,
    duration: "15",
    degree: "15"
...
let getLessons = (grade, subject) => {
    axios.get(route('tests.lessonsOfSubject', {grade: grade, subject: form.subject}))
...
let getTestsTypes = (grade, subject) => {
    axios.get(route('tests.testTypesOfSubjectGrade', {grade: grade, subject: form.subject}))
